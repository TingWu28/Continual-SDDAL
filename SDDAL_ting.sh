...
# ==========================================
# Usage:
#   bash SDDAL_ting.sh <beamshape> <lr> <initial_size> <init_only?> <start_round> <end_round> <gpu> <scanner_batch_size> <retrain_frequency> <scan_only?> <update_size> <test_data> <buffer_size>
#
# Example:
#   regular scheme: bash SDDAL_ting.sh rec 0.0002 100 false 1 580 0 5 1 false 50 Design_rec 500
#   eval disabled:  bash SDDAL_ting.sh rec 0.0002 100 false 1 580 0 5 1 false 0 "" 500
#   scan-only scheme: run the following sequence to do 1000initial+1000SDDAL
#                   bash SDDAL_ting.sh rec 0.0002 1000 true 9999 9999 0 9999 9999 false 0 "" 500
#                   bash SDDAL_ting.sh rec 9999 9999 false 1 200 0 5 9999 true 0 "" 500
#
...
#   buffer_size : max samples kept in the replay buffer (default 500)
#                 AL loop uses ContinualTrainer.py (warm-start + replay)
#                 init_only block still uses Trainer.py (cold start)
# ==========================================
...
buffer_size=${13:-500}   # replay buffer capacity for ContinualTrainer
    ...
    if [ "${scan_only}" = false ]; then
        if (( (round_sampling - 1) % retrain_freq == 0 )); then
            ...
            python3 ContinualTrainer.py \
                ...
                --seed        123 \
                --pth_name    Design_${beamshape}/models/QuantUNetT_${beamshape} \
                --resume      Design_${beamshape}/models/QuantUNetT_${beamshape} \
                --buffer_path Design_${beamshape}/models/replay_buffer.pkl \
